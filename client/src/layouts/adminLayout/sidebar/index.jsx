...
const Sidebar = () => {
  ...
  return (
    <div
      ...
      } ${menuState ? "left-0" : "-left-[200%] lg:left-0"}`}
    >
      <div className="relative flex flex-col gap-3 px-5">
        ...
        <span
          ...
          className={`absolute top-3 md:top-3 lg:top-1/2 right-2 md:right-0 -translate-y-1/2 h-8 aspect-square md:aspect-auto md:h-12 flex items-center justify-center cursor-pointer text-lg md:text-xl rounded-full md:rounded-l-lg lg:hidden ${
            theme === "dark" ? "bg-darkBg" : "bg-indigo-500/5"
          }`}
        >
      ...
      <ul className="flex flex-col gap-3 mt-8 capitalize">
        {sidebar.map((items) => (
          <>
            <li
              key={items.title}
              className={`mt-1 uppercase font-semibold text-sm px-5 ${
              ...
              {items.links.map((link) => {
                ...
                return (
                  <li key={link.path}>
                    <Link
                      to={link.path}
                      className={`w-full px-5 py-4 2xl:text-base text-[0.9rem] hover:bg-indigo-500/10 flex items-center gap-1 relative before:absolute before:top-0 before:right-0 before:w-1 before:h-full before:bg-indigo-600 before:rounded-full before:scale-0 hover:before:scale-100 before:transition-all before:duration-500 ${
                        theme === "dark"
                          ? "hover:text-white"
                          : "hover:text-indigo-600"
                      } ${
                        currentPath === link.path.split("/")[2] &&
                        `${
                          theme === "dark"
                            ? "text-white bg-indigo-500/10 before:scale-100"
                            : "bg-indigo-500/10 text-indigo-600 before:scale-100"
                        }`
                      }`}
                    >
